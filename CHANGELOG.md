...
- fix(redis): pool options no longer reach `Redix.command/3`. A Redis cache configures its poolboy pool in the same keyword list as its `uri` (`size:`, `max_overflow:`, `strategy:`), and `Cache.Redis.child_spec/1` split those out correctly, but the command path dropped only `:uri` and handed the rest to Redix. Redix 1.5 onward validates command options with `NimbleOptions` and raises on anything outside `[:timeout, :telemetry_metadata]`, so every read and write on a cache configured with `size:` crashed with `** (NimbleOptions.ValidationError) unknown options [:size]`. The pool-only keys are now dropped alongside `:uri` before running a command.
- fix(multi_layer): `Cache.MultiLayer` rejects non-module layers at startup instead of silently no-opping. A bare adapter (`Cache.ETS`) or an `{Adapter, opts}` tuple was documented as a valid layer but never dispatched — the fallback branches returned `{:ok, nil}` and `:ok`, so an inert layer was indistinguishable from a cold cache. Layers must be cache modules (`use Cache`); anything else raises naming the offending element and how to wrap it. A cache also cannot be its own layer, since its `get/1` is this strategy's `get/4` and the cascade would recurse.
