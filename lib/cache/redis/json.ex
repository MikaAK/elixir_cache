defmodule Cache.Redis.JSON do
  ...
  def get(pool_name, key, path, opts) do
    with {:ok, data} <- json_command(pool_name, key, "GET", get_command(path), opts) do
      {:ok, TermEncoder.decode_json(data)}
    end
  end

  defp get_command(nil) do
    []
  end

  defp get_command(path) do
    [serialize_path(path)]
  end

  def set(pool_name, key, nil, value, opts) do
    set(pool_name, key, "$", value, opts)
  end
  ...
  defp serialize_path(path) when is_binary(path) do
    path
  end
  ...
  defp json_command(pool_name, key, command, commands, opts) do
    with {:ok, value} <- Redis.Global.command(pool_name, [
      "JSON.#{command}",
      Redis.Global.cache_key(pool_name, key) | commands
    ], opts) do
      if is_binary(value) and value =~ ~r/^\d+$/ do
